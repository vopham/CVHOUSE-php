...
//$query = "SELECT * FROM customer_order Where order_status = 'Đã xác nhận' ORDER  BY order_id";
        ...
        <div class="col-md-9">
            <div class="row">
                ...
                <div class="col-md-11 text-left mt-4">
                    <h1 class="ml-5 display-4 font-weight-normal">Đơn hàng đã xác nhận:</h1>
                </div>
            </div>
            ...
            <div class="col-md-6"><td>
                  <?php 
                  $sql1 = ("SELECT count(order_id) as total from customer_order where order_status ='Đã xác nhận'");
                  $result=mysqli_query($con,$sql1);
                  $data=mysqli_fetch_assoc($result);
                  echo "Tổng đơn hàng đã xác nhận: " ;echo $data['total']; echo " đơn hàng" ?>
                </td></div>
                ...
                <table class="table table-responsive table-hover ">
                    ...
                    <tbody class="text-center">
                        ...
                        if (isset($_POST['sear_verified_furiture_pro'])) {
                            ...
                            $order_query1 = "SELECT * FROM customer_order WHERE order_status='Đã xác nhận' ";
                            if (isset($_GET['page'])) {
                                $page_id = $_GET['page'];
                            } else {
                                $page_id = 1;
                            ...
                            $pages = ceil($count_rows / $required_pro);
                            $oder_start = ($page_id - 1) * $required_pro;
                            $order_query = "SELECT * FROM customer_order WHERE order_status='Đã xác nhận'  ORDER BY order_id DESC LIMIT $oder_start,$required_pro";
                        }
                        ...
                        <?php
                            ...
                            echo "<tr><td colspan='12'><h2 class='text-center text-secondary'>Không tìm thấy đơn hàng đã xác nhận</h2></td></tr>";
                        }
            ...
            <ul class="pagination pagination-md mt-5">
                ...
                for ($i = 1; $i <= $pages; $i++) {
                    echo "<li class='page-item " . ($i == $page_id ? ' active ' : '') . "'><a class='page-link' href='verified_furniture_pro.php?page=$i'>$i</a></li>";
                } ?>
            </ul>
